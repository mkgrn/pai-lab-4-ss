...
class PortalFront extends Portal
{
  ...
  function login()
  {
    ...
    if (
      $userEmailLength < 5 || $userEmailLength > 250 ||
    ...
    if (!$result = $this->dbo->query($query)) {
      setMessage('Wystąpił błąd: nieprawidłowe zapytanie...');
      return SERVER_ERROR;
    }
    //Sprawdzenie wyników zapytania
    if ($result->num_rows <> 1) {
      setMessage("Wystąpił błąd: brak użytkownika o wskazanej nazwie");
      return ACTION_FAILED;
    } else {
      $row = $result->fetch_row();
      $pass_db = $row[3];
      if ($pass != $pass_db) { // Wersja bez kodowania haseł
        setMessage("Wystąpił błąd: podano niepoprawne hasło");
        return ACTION_FAILED;
      } else {
        $nazwa = $row[1] . ' ' . $row[2];
  ...
  function logout()
  {
    ...
    session_destroy(); // Treść meotdy logout
  }
